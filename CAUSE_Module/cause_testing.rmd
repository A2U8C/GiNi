...
```{r}
Sys.setenv(PATH = "//smb-ifs.ini.usc.edu/faculty/njahansh/nerds/ankush/software/Anaconda/envs/r_env_ankush")
...
input_trait_Enigma_clump <- input_trait_Enigma %>%
  ...
  ieugwasr::ld_clump(dat = .,
                     clump_r2 = r2_thresh,
                     clump_p = pval_thresh,
                     plink_bin = "//smb-ifs.ini.usc.edu/faculty/njahansh/nerds/ankush/software/Anaconda/envs/r_env_ankush/lib/R/library/genetics.binaRies/bin/plink",
...
res_input_trait_Enigma_snps <- res_input_trait_Enigma$data %>% filter(snp %in% top_vars_input_trait_Enigma[which(top_vars_input_trait_Enigma %in% (input_trait_Enigma %>% filter(p1 < 5e-8) %>% select(snp))$snp)])

print(res_input_trait_Enigma_snps)
...
```{r}
#Load Data
load("//smb-ifs.ini.usc.edu/faculty/njahansh/nerds/ravi/ABCD/Pain/CAUSE.RData")
...
```{r}
process_data <- function(res_snp_df) {
...
res_copc_anx_plot <- plot(res_copc_anx, denormalized_df_sub_chr, type="data")# + labs(y = "Association with Anxiety (b)", x = "Association with Chronic Widespread Pain (b)", title = "")
```


```{r}
print(denormalized_df_sub_chr)
```