...
n.parallel = 5#5
if (qtl == "sqtl") {
  if (tissue %in% high.mem | ((chr==19 | chr==16 | chr==8 | chr==9) & tissue %in% high.mem.chrs )) { n.parallel = 5 }#5
}
print(paste('Using',n.parallel,'cores!'))

### Read in data
gwas.input = process.input(info.file, sample.overlap.file=NULL, ref.dat)
process.eqtl.input(gwas.input, qtl, tissue=tissue, is.sqtl=ifelse(qtl=="/ifs/loni/faculty/njahansh/nerds/ravi/genetics/LAVA-main/sqtl", T, F), chromosomes=chr)
...
out = mclapply(1:length(genes), mc.cores=n.parallel, function(i) {
  ...
  if ("sqtl" %in% names(locus$binary)) {
  if (!is.null(locus)) {
    loc.info = data.frame(locus = locus$id, chr = locus$chr, n.snps = locus$n.snps, n.pcs = locus$K)

    # run univ and bivar
    ub = run.univ.bivar(locus, univ.thresh=univ.thresh, target = "sqtl")
